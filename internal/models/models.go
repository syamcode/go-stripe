...
func (m *DBModel) InsertTransaction(txn Transaction) (int, error) {
	...
	stmt := `
		insert into transactions
			...
			transaction_status_id, payment_intent, payment_method, created_at, updated_at)
		values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
...
func (m *DBModel) UpdatePasswordForUser(u User, password string) error {
	...
	_, err := m.DB.ExecContext(ctx, "update users set password = ? where id = ?", password, u.ID)
	if err != nil {
		return err
	}
...
func (m *DBModel) UpdateOrderStatus(orderID, status int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	_, err := m.DB.ExecContext(ctx, "update orders set status_id = ? where id = ?", status, orderID)
	if err != nil {
		return err
	}
...
func (m *DBModel) EditUser(u User) error {
	...
	query := `
		...
		where 
			id = ?
	`
	_, err := m.DB.ExecContext(ctx, query, u.LastName, u.FirstName, u.Email, time.Now(), u.ID)
	if err != nil {
		return err
	}
...
func (m *DBModel) AddUser(u User) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
